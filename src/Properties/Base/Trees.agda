...
open import Data.List.Relation.Binary.Permutation.Propositional.Properties using (∈-resp-↭; map⁺; shift; ++-comm; ↭-empty-inv; All-resp-↭)
...
opaque
  ...
  noMsgsForUnknownParty N₀↝⋆N p∉ps₀ = noMsgsForUnknownPartyʳ (Star⇒Starʳ N₀↝⋆N) p∉ps₀
    where
      open RTC; open Starʳ

      noMsgsForUnknownPartyʳ : ∀ {p : Party} {N : GlobalState} →
          N₀ ↝⋆ʳ N
        → p ∉ parties₀
        → L.All.All ((_≢ p) ∘ rcv) (N .messages)
      noMsgsForUnknownPartyʳ εʳ _ = []
      noMsgsForUnknownPartyʳ {p} {N} (_◅ʳ_ {j = N″} N₀↝⋆ʳN″ N″↝N) p∉ps₀ = goal N″↝N
        where
          N₀↝⋆N″ : N₀ ↝⋆ N″
          N₀↝⋆N″ = Starʳ⇒Star N₀↝⋆ʳN″

          ih : L.All.All ((_≢ p) ∘ rcv) (N″ .messages)
          ih = noMsgsForUnknownPartyʳ N₀↝⋆ʳN″ p∉ps₀

          p∉eoN″ : p ∉ N″ .execOrder
          p∉eoN″ p∈eoN″ = contradiction (∈-resp-↭ (↭-sym $ execOrderPreservation-↭ N₀↝⋆N″) p∈eoN″) p∉ps₀

          goal : N″ ↝ N → L.All.All ((_≢ p) ∘ rcv) (N .messages)
          goal (deliverMsgs {N′ = N‴} N″Ready N″—[eoN″]↓→∗N‴) = goal* (reverseView (N″ .execOrder)) p∉eoN″ (—[]→∗⇒—[]→∗ʳ N″—[eoN″]↓→∗N‴)
            where
              open import Data.List.Reverse

              goal* : ∀ {N* ps} → Reverse ps → p ∉ ps → _ ⊢ N″ —[ ps ]↓→∗ʳ N* → L.All.All ((_≢ p) ∘ rcv) (N* .messages)
              goal* [] _ ts rewrite sym $ —[[]]→∗ʳ⇒≡ ts = ih
              goal* {N*} (ps′ ∶ ps′r ∶ʳ p′) p∉[ps′+p′] N″—[ps′+p′]↓→∗ʳN*
              ...
              ... | N° , N″—[ps′]↓→∗N° , N°—[p′]↓→N* = step N°—[p′]↓→N*
                where
                  ...
                  ih* : L.All.All ((_≢ p) ∘ rcv) (N° .messages)
                  ih* = goal* ps′r p∉ps′ (—[]→∗⇒—[]→∗ʳ N″—[ps′]↓→∗N°)

                  step : _ ⊢ N° —[ p′ ]↓→ N* → L.All.All ((_≢ p) ∘ rcv) (N* .messages)
                  ...
                  ... | newMds , _ = goalᶜ newMds
                    where
                      ...
                      goalᶜ : ∀ mds* → L.All.All ((_≢ p) ∘ rcv) (Nᶜ mds* .messages)
                      goalᶜ [] = L.SubS.All-resp-⊇ (L.SubS.filter-⊆ _ _) ih*
                      goalᶜ ((m , φ) ∷ mds)
                        rewrite
                          sym $ execOrderPreservation-≡-broadcastMsgsᶜ mds (removeImmediateMsgs p′ N°)
                          = L.All.++⁺ (All-resp-↭ (map⁺ _ eoN″↭eoN°) (goalᶜ-∷ _ p∉eoN″)) (goalᶜ mds)
                        where
                          eoN″↭eoN° : N″ .execOrder ↭ N° .execOrder
                          eoN″↭eoN° = execOrderPreservation-↭-↓∗ N″—[ps′]↓→∗N°

                          goalᶜ-∷ : ∀ ps* → p ∉ ps* → L.All.All ((_≢ p) ∘ rcv) (L.map (λ p* → ⦅ m , p* , φ p* .value ⦆) ps*)
                          goalᶜ-∷ [] _ = []
                          goalᶜ-∷ (p* ∷ ps*) p∉p*+ps* with ∉-∷⁻ p∉p*+ps*
                          ... | p≢p* , p∉ps* = ≢-sym p≢p* ∷ goalᶜ-∷ ps* p∉ps*
          goal (makeBlock {N′ = N‴} N′MsgsDelivered N″—[eoN″]↑→∗N‴) = goal* $ —[]→∗⇒—[]→∗ʳ N″—[eoN″]↑→∗N‴
            where
              goal* : ∀ {N* ps} → _ ⊢ N″ —[ ps ]↑→∗ʳ N* → L.All.All ((_≢ p) ∘ rcv) (N* .messages)
              goal* [] = ih
                ...
                where
                  ...
                  step (honestParty↑ {ls = ls} _ _) = goalʰ $ mb .proj₁
                    where
                      ...
                      goalʰ : ∀ ms → L.All.All ((_≢ p) ∘ rcv) (N°ʰ ms .messages)
                      goalʰ [] = ih*
                      goalʰ (m ∷ ms)
                        rewrite
                          sym $ execOrderPreservation-≡-broadcastMsgsʰ ms (updateLocalState p′ (mb .proj₂) N°)
                          = L.All.++⁺ ((All-resp-↭ (map⁺ _ eoN″↭eoN°) (goalʰ-∷ _ p∉eoN″))) (goalʰ ms)
                        where
                          eoN″↭eoN° : N″ .execOrder ↭ N° .execOrder
                          eoN″↭eoN° = execOrderPreservation-↭-↑∗ (—[]→∗ʳ⇒—[]→∗ ts⋆)

                          goalʰ-∷ : ∀ ps* → p ∉ ps* → L.All.All ((_≢ p) ∘ rcv) (L.map (λ p* → ⦅ m , p* , 𝟙 ⦆) ps*)
                          goalʰ-∷ [] _ = []
                          goalʰ-∷ (p* ∷ ps*) p∉p*+ps* with ∉-∷⁻ p∉p*+ps*
                          ... | p≢p* , p∉ps* = ≢-sym p≢p* ∷ goalʰ-∷ ps* p∉ps*
          ...
          goal (advanceRound _) = goal* (N″ .messages) ih
            where
              goal* : ∀ ms → L.All.All ((_≢ p) ∘ rcv) ms → L.All.All ((_≢ p) ∘ rcv) (L.map decreaseDelay ms)
              goal* [] _ = []
              goal* (m ∷ ms) [rcv≢p][m+ms] = L.All.head [rcv≢p][m+ms] ∷ goal* ms (L.All.tail [rcv≢p][m+ms])
