        ...
        goal-↓∗ :
          ...
          ∃₂[ sl′ , sl″ ]
            ...
            × length (superSlotsInRange (sl′ + 1) (sl″ ∸ 1))
              ≤
              2 * length (corruptSlotsInRange (sl′ + 1) (sl″ + 1))
        goal-↓∗ rewrite clockPreservation-↓∗ N′—[eoN′]↓→∗N″ = (case ih of λ where
          (inj₁ bc₁⪯bcN′ls₂′) → (
    ...
    ... | makeBlock {N′} {N″} N′MsgsDelivered N′—[eoN′]↑→∗N″ = goal-↑∗
      where
        ...
        subgoal-⪯-↑∗ (⁇ (yes isWinner)) bc₁⪯bcN′ls₂′ =
            case singlePartyCommonPrefix {k = k} N₀↝⋆N′ ffN′ cfN′ {p₂} {ls₂′} hp₂ lsp₂N′ {bcN′ls₂} {0} π1 π2 π3 of λ where
              (inj₁ bcN′ls₂′⪯bcN′ls₂) → inj₁ $ prune-⪯-trans {c₁ = bc₁} bc₁⪯bcN′ls₂′ bcN′ls₂′⪯bcN′ls₂
          ...
          where
            ...
            opaque
              ...
              lsp₂N*p₂≡ls₂′+nb : N*p₂ .states ⁉ p₂ ≡ just (addBlock ls₂′ nb)
            ...
            π3 : ∣ bcN′ls₂′ ∣ ≤ ∣ bcN′ls₂ ∣
            π3 = subst (λ ◆ → ∣ bcN′ls₂′ ∣ ≤ ∣ bestChain (N′ .clock ∸ 1) (◆  .tree) ∣) ls₂′+nb≡ls₂ π3-1
              where
                bcN′ls₂′+nb = Chain ∋ bestChain (N′ .clock ∸ 1) (extendTree (ls₂′ .tree) nb)

                π3-1 : ∣ bcN′ls₂′ ∣ ≤ ∣ bcN′ls₂′+nb ∣
                π3-1 = optimal bcN′ls₂′ (extendTree (ls₂′ .tree) nb) (N′ .clock ∸ 1) bcN′ls₂′✓ π3-1-1
                  where
                    bcN′ls₂′✓ : bcN′ls₂′ ✓
                    bcN′ls₂′✓ = valid (ls₂′ .tree) (N′ .clock ∸ 1)

                    π3-1-1 : bcN′ls₂′ ⊆ˢ filter ((_≤? N′ .clock ∸ 1) ∘ slot) (allBlocks (extendTree (ls₂′ .tree) nb))
                    π3-1-1 = begin
                      bcN′ls₂′
                        ⊆⟨ selfContained (ls₂′ .tree) (N′ .clock ∸ 1) ⟩
                      filter ((_≤? N′ .clock ∸ 1) ∘ slot) (allBlocks (ls₂′ .tree))
