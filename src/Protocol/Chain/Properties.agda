...
opaque
  ...
  [b]✓⇔b≡gb = mk⇔ to′ from′
    where
      to′ : ∀ {b : Block} → [ b ] ✓ → b ≡ genesisBlock
      to′ = proj₁ ∘ proj₂

      from′ : ∀ {b : Block} → b ≡ genesisBlock → [ b ] ✓
      from′ b≡gb rewrite b≡gb = genesisWinner ∷ [] , refl , [-]

  [gb+c]✓⇔c≡[] : ∀ {c : Chain} → (genesisBlock ∷ c) ✓ ⇔ c ≡ []
  [gb+c]✓⇔c≡[] = mk⇔ to′ from′
    where
      to′ : ∀ {c : Chain} → (genesisBlock ∷ c) ✓ → c ≡ []
      to′ {[]} _ = refl
      to′ {b ∷ c} [gb∷b∷c]✓ = contradiction genesisBlockSlot (Nat.n>0⇒n≢0 $ Nat.m<n⇒0<n $ ✓-∷ .from [gb∷b∷c]✓ .proj₂ .proj₂ .proj₁)

      from′ : ∀ {c : Chain} → c ≡ [] → (genesisBlock ∷ c) ✓
      from′ c≡[] rewrite c≡[] = [b]✓⇔b≡gb .from refl
