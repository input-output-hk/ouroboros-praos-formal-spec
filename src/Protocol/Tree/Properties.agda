...
module Protocol.Tree.Properties
...
module _ {T : Type} ⦃ _ : Tree T ⦄ where
  ...
  buildTreeUsesAllBlocks [] = ≡⇒≡ˢ $ instantiated {T = T}
  buildTreeUsesAllBlocks (b ∷ bs) {b′} = begin
    b′ ∈ allBlocks (buildTree (b ∷ bs))          ≡⟨⟩
    b′ ∈ allBlocks (extendTree (buildTree bs) b) ∼⟨ extendable {T = T} _ _ ⟩
  ...
  allBlocksBuildTree-++ [] bs′ {b} = let open Related.EquationalReasoning in begin
    ...
    b ∈ [ genesisBlock ] ++ allBlocks (buildTree bs′)          ∼⟨ ++-cong (≡⇒≡ˢ $ sym (instantiated {T = T})) ≡ˢ-refl ⟩
    b ∈ allBlocks {T = T} tree₀ ++ allBlocks (buildTree bs′)   ∎
  allBlocksBuildTree-++ (b ∷ bs) bs′ {b′} = let open Related.EquationalReasoning in begin
    b′ ∈ allBlocks (buildTree ((b ∷ bs) ++ bs′))                              ≡⟨⟩
    b′ ∈ allBlocks (extendTree (buildTree (bs ++ bs′)) b)                     ∼⟨ extendable {T = T} _ _ ⟩
    ...
    b′ ∈ (allBlocks (buildTree bs) ++ [ b ]) ++ allBlocks (buildTree bs′)     ∼⟨ ++-cong
                                                                                  (≡ˢ-sym $ extendable {T = T} _ _)
                                                                                  ≡ˢ-refl ⟩
