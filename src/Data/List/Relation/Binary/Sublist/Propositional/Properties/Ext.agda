...
All-resp-≡ : ∀ {A : Set a} {x y : A} {xs : List A}
           → x ≡ y → All (_≢_ x) xs → All (_≢_ y) xs
All-resp-≡ refl pxs = pxs

Unique-resp-⊇ : Unique {A = A} Respects _⊇_
Unique-resp-⊇ []            []         = []
Unique-resp-⊇ (_ ∷ʳ ys⊆xs)  (_ ∷ uxs)  = Unique-resp-⊇ ys⊆xs uxs
Unique-resp-⊇ (x≡y ∷ ys⊆xs) (px ∷ uxs) = All-resp-≡ (sym x≡y) (All-resp-⊆ ys⊆xs px) ∷ Unique-resp-⊇ ys⊆xs uxs
